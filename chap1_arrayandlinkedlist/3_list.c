...
mylist *newlist()
{
    mylist *nums = (mylist *)malloc(sizeof(mylist));         //在c语言的较新标准中，推荐将malloc返回的通用指针void*显式转化为目标类型的指针
    nums->capacity = 10;                                     //初始容量设置为10
    nums->arr = (int *)malloc(sizeof(int) * nums->capacity); // arr接受存放列表元素的堆区地址，本质上是通过数组来管理列表元素
...
{
    assert(index >= 0 && index < nums->size); //这里利用了c标准库<assert.h>中的assert函数，来判断index是不是合法索引
...
void insert(mylist *nums, int index, int num)
{
    ...
    {
        nums->arr[i] = nums->arr[i - 1]; //顺带一提，在索引2处插入元素，是在索引1的后面插入，即新元素的下标为2
    }
    nums->arr[index] = num;
    nums->size++;
}
...
void extendcapacity(mylist *nums)
{
    ...
    int *temp = nums->arr;                                  //方便后续释放不再需要的旧数组内存。nums->arr稍后会将扩容后的数组的新地址存放在内，所以需要先释放旧地址内存，避免内存泄漏
...
int main()
{
    ...
    for (int i = 0; i < 10; i++)
    {
        add(nums, i); //在列表尾部添加元素i。测试前，列表的长度为5，在i=5时，add函数会判断此时列表的实际大小已经等于列表容量，会触发扩容机制之后再到尾部添加元素5
    }
    printf("扩容后的列表 nums = ");
    printArray(toarray(nums), size(nums));
    printf("容量 = %d,实际长度 = %d\n", capacity(nums), size(nums));
