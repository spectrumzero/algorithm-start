...
{
    if (!n0->next)
        return; //如果条件为真，则说明n0->next = null,说明传入的节点已经是最后一个节点，返回。而这里要执行的命令只有一条，可以省略原有的中括号。
    // n0->p->n1
    ListNode *p = n0->next;
    ListNode *n1 = p->next;
...
ListNode *access(ListNode *head, int index)
{
    ...
    for (int i = 0; i < index; i++)
    {
        if (head == NULL) //链表不存在
            return NULL;
        head = head->next; //寻下一个节点。这里的“索引”，可以类似数组一样理解，但链表严格来说并没有index的概念
    }
    return head;
}
...
int main()
{
    ...
    /*插入节点*/
    insert(n3, newListNode(100)); //第二个参数表示这个函数所返回的新节点（这里也就是要插入的节点）的地址
    printf("插入节点后的链表为\r\n");
    printLinkedList(n0);
