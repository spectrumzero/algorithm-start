...
int randomaccess(int *nums, int size) {
  ...
  srand(time(
      NULL)); // 设定一个随机种子，确保是真正随机的。在使用rand函数之前，调用c标准库中的另一个函数，利用时间去随机化伪随机数生成器的种子。如果不调用，生成器会使用固定的种子值，达不到随机的效果。
...
int main() {
  ...
  /*随机访问*/
  int randomnum = randomaccess(
      nums,
      size); // 注意，在主函数中的randomnum和randomaccess函数最后返回的randomnum，是两个变量。
  printf("随机访问得到的元素为：%d\n", randomnum);
  ...
  int *res = extend(
      nums, size,
      enlarge); // 新建了一个指针型变量，接收extend函数的返回值，也就是位于堆区的、扩展了长度之后的数组。
  size += enlarge;
