...
{
  arrayqueue *queue = (arrayqueue *)malloc(sizeof(
      arrayqueue)); // 熟悉的malloc函数。同样的，将新的队列开在堆区，利用指针进行管理。
  // 初始化数组。
  queue->quecapacity = capacity; // 数组的容量，即队列可容纳的元素容量
  queue->nums = (int *)malloc(
      sizeof(int) *
      queue->quecapacity); // nums为指针型变量。单个元素的大小*可容纳的元素量
                           // = 应该开辟的内存大小
...
{
  assert(size(queue) !=
         0); // 不为空才继续下去，需要括号内为真。这里是通过size函数来判断的。
...
int pop(arrayqueue *queue)
{
  ...
  // 因此，原来的打印函数是有误的。要重写。
  queue->front =
      (queue->front + 1) % queue->quecapacity; // front+1的威力，就是整体平移。
  queue->quesize--;
  return num;
}
...
int main()
{
  ...
  for (int i = 0; i < 21; i++)
  {
    push(queue, i);
    pop(queue);
    printf("第 %d 轮入队 + 出队后 queue = ", i); // 在第3轮入队的时候，是num[0]被重新赋值为3。num[10]只是在逻辑上合法，在区间上合法，但是语法不合法。
    printArray(queue->nums, queue->quesize, queue->front, queue->quecapacity);
  } // 以front为中心来理解。
