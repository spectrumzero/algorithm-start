...
void push(queuelinkedlist *queue, int num) {
  ...
  // 如果队列不为空，则将该节点添加到尾节点后
    ...
    queue->rear->next =
        node; // 首先是让目前的最后节点指向新节点，完成链接。省略了这一步，将会使得oldrear->next依旧为null，发生断链，且无法寻找到newrear。
    queue->rear =
        node; // 然后是让这个新节点实际成为最后的节点。这个时候，newrear->next = null是合法的。
  }
  queue->quesize++;
}
...
void printqueuelinkedlist(queuelinkedlist *queue) {
  ...
  int *arr = malloc(sizeof(int) * queue->quesize); // 新开辟一块数组,用于存放拷贝的数据
